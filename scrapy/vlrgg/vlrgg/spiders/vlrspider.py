    ...
    def postparse(self, response):
        ...
        for map in mapdata:
            ...
            if not map.attrib['data-game-id']=='all':
                ...
                mapdf = pd.DataFrame(columns=['round', 'winningTeamRound', 'losingTeamRound', 'date', 'map', 'event', 'winner', 'loser'])
                    ...
                    if len(col.css('.rnd-sq'))==2:
                        ...
                        if "win" in team1_classnames:
                            ...
                            if not col.css('.rnd-sq')[0].css('img')[0].attrib['src'] =='/img/vlr/game/round/elim.webp':
                                tempnum = tempnum +1
                            mapdf = mapdf.append({'round': counter, 'winningTeamRound': tempnum, 'losingTeamRound': 0,'date': date.strftime('%d-%m-%y'), 'map': mapName, 'event': response.meta['event'], 'winner':winner, 'loser':loser}, ignore_index=True)
                        else:
                            if "win" in team2_classnames:
                                ...
                                if not col.css('.rnd-sq')[1].css('img')[0].attrib['src'] =='/img/vlr/game/round/elim.webp':
                                    tempnum = tempnum + 1
                                mapdf = mapdf.append({'round': counter, 'winningTeamRound': 0, 'losingTeamRound': tempnum, 'date': date.strftime('%d-%m-%y'), 'map': mapName, 'event': response.meta['event'], 'winner':winner, 'loser':loser}, ignore_index= True)
                        counter = counter +1
